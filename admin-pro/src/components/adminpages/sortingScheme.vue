  ...
  <div class="wrapper">
    ...
    <div :style="{ height: bodyHeight - 250 + 'px' }" class="contentBody">
      ...
      >
        ...
        <el-table-column
          align="center"
          prop="collectFlagName"
          label="是否上传(揽收)"
        ></el-table-column>
        <el-table-column
          align="center"
          prop="dispatchFlagName"
          label="是否派件(揽收)"
        ></el-table-column>
        <el-table-column
          align="center"
          prop="boardingFlagName"
          label="是否分部(上车)"
        ></el-table-column>
        <el-table-column
          align="center"
          prop="packagesFlagName"
          label="是否建包"
        ></el-table-column>
        <el-table-column
          align="center"
    ...
    <div class="showAddToast">
      <el-dialog title="添加" width="40%" :visible.sync="dialogFormVisible">
        <el-form :rules="form.rules" ref="form" :model="form">
          ...
          >
            <el-input
              placeholder="请输入实际目的网点"
              v-model="form.actualOrgCode"
              autocomplete="off"
            ></el-input>
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
        ...
        that
          ...
          .then(() => {
            ...
            that
              .$axios({
                method: "post",
                url: sessionStorage.getItem("IP") + "sortingPlanInfo/import",
                headers: {
    ...
    getData() {
      that
        .$post("sortingPlanInfo/getSortingPlanInfoPage", {
    ...
    submitForm(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          that
            .$post("sortingPlanInfo/editSortingPlanInfo", {
              //   id: that.thisRowId,
    ...
    //编辑
    submitForm2(form2) {
      for (var i = 0; i < that.form.expressList.length; i++) {
        if (
          that.form2.expressTypeName == that.form.expressList[i].dictionaryValue
        ) {
          that.form2.expressType = that.form.expressList[i].dictionaryKey;
        }
      }
    ...
    if (that.asideData) {
      for (var ti = 0; ti < that.asideData.length; ti++) {
        for (var tis = 0; tis < that.asideData[ti].chrildList.length; tis++) {
          if (that.asideData[ti].chrildList[tis].pageUrl == "sortingScheme")
            for (
              var tiss = 0;
