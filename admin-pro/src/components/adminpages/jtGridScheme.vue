...
export default {
  ...
  methods: {
    ...
    getData() {
      that
        .$post("sortingPlanJt/getSortingPlanJtPage", {
        ...
        .then(function (data) {
          if (data && data.code == "0000") {
            ...
            for (let i = 0; i < data.page.list.length; i++) {
              if (data.page.list[i].flowDirection === 0) {
                that.tableData[i].flowDirection = "进港";
              } else {
                that.tableData[i].flowDirection = "出港";
              }
            }
            that.$nextTick(() => {
    ...
    submitForm(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          that
            .$post("sortingPlanJt/editSortingPlanJt", {
              id: that.thisRowId,
    ...
    handleClickEdit(row) {
      ...
      that.form2.flowDirection = row.flowDirection == 1 ? "出港" : "进港";
    ...
    if (that.asideData) {
      for (var ti = 0; ti < that.asideData.length; ti++) {
        for (var tis = 0; tis < that.asideData[ti].chrildList.length; tis++) {
          if (that.asideData[ti].chrildList[tis].pageUrl == "jtGridScheme")
            for (
              var tiss = 0;
