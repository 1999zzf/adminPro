...
export default {
  ...
  data() {
    return {
      ...
      form3: {
        ...
        rules: {
          dictionaryKey: [
            { required: false, message: "请输入字典key", trigger: "blur" },
          ],
          dictionaryValue: [
            { required: false, message: "请输入字典值", trigger: "blur" },
          ],
          dictionaryType: [
            { required: false, message: "请输入字典类型", trigger: "blur" },
          ],
        },
      },
      thisRowId: "",
  ...
  methods: {
        ...
        that
          ...
          .then(() => {
            ...
            that
              .$axios({
                method: "post",
                url: sessionStorage.getItem("IP") + "sortingPlanYto/import",
                headers: {
    ...
    getData() {
      that
        .$post("sortingPlanYto/getSortingPlanYtoPage", {
    ...
    submitForm(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          that
            .$post("sortingPlanYto/editSortingPlanYto", {
              id: that.thisRowId,
    ...
    if (that.asideData) {
      for (var ti = 0; ti < that.asideData.length; ti++) {
        for (var tis = 0; tis < that.asideData[ti].chrildList.length; tis++) {
          if (that.asideData[ti].chrildList[tis].pageUrl == "ytGridScheme")
            for (
              var tiss = 0;
