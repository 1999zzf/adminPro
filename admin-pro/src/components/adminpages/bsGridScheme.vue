...
export default {
  ...
  methods: {
        ...
        that
          ...
          .then(() => {
            ...
            that
              .$axios({
                method: "post",
                url: sessionStorage.getItem("IP") + "sortingPlanBs/import",
                headers: {
    ...
    getData() {
      that
        .$post("sortingPlanBs/getSortingPlanBsPage", {
    ...
    submitForm(form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          that
            .$post("sortingPlanBs/editSortingPlanBs", {
              id: that.thisRowId,
    ...
    if (that.asideData) {
      for (var ti = 0; ti < that.asideData.length; ti++) {
        for (var tis = 0; tis < that.asideData[ti].chrildList.length; tis++) {
          if (that.asideData[ti].chrildList[tis].pageUrl == "bsGridScheme")
            for (
              var tiss = 0;
...
@import "../../assets/css/adminpages/bsGridScheme.scss";
</style>